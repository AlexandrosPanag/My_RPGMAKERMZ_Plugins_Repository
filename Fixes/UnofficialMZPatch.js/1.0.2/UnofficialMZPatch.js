...
(() => {
    ...
    */
    ...
    //=========================================================================
// SECTION 8F: SMOOTH SCENE TRANSITIONS (FIXED - Alt-Tab Safe)
//=========================================================================
...
const TransitionManager = {
    ...
    // **NEW: Handle window visibility changes**
    onVisibilityChange() {
        if (document.hidden) {
            // Window lost focus - force complete any active fade
            if (this.fading) {
                this.completeFade();
            }
        } else {
            // Window regained focus - ensure fade sprite is removed
            this.removeFadeSprite();
        }
    }
};
...
document.addEventListener('visibilitychange', () => {
    TransitionManager.onVisibilityChange();
});

// **NEW: Also listen for blur/focus events as backup**
window.addEventListener('blur', () => {
    if (TransitionManager.fading) {
        TransitionManager.completeFade();
    }
});

window.addEventListener('focus', () => {
    TransitionManager.removeFadeSprite();
});
