...
(() => {
    ...
    try {
        ...
        console.warn(`[${pluginName}] Could not load parameters, using defaults`);
    }
    
    // Log that plugin is starting
    console.log(`[${pluginName}] v${VERSION} initializing...`);
    ...
    if (Config.enableCollisionFixes && typeof Game_Player !== 'undefined') {
        ...
        console.log(`[${pluginName}] Diagonal collision fixes applied`);
    }
    ...
    if (Config.enableInputFixes && typeof Input !== 'undefined') {
        ...
        console.log(`[${pluginName}] Input fixes applied`);
    }
        ...
        SceneManager.cleanupBitmapCache = function() {
            ...
            if (totalSize > maxSize) {
                ...
                if (Config.enableErrorLogging && removedSize > 0) {
                    console.log(`[${pluginName}] Bitmap cache cleanup: Removed ${(removedSize / 1024 / 1024).toFixed(2)}MB`);
                }
            }
    ...
    if (Config.enableAudioFixes && typeof AudioManager !== 'undefined') {
        ...
        console.log(`[${pluginName}] Audio fixes applied`);
    }
    ...
    if (Config.enableSaveSystemFixes && typeof DataManager !== 'undefined') {
        ...
        DataManager.loadGame = function(savefileId) {
            return _DataManager_loadGame.call(this, savefileId).catch(e => {
                console.error(`[${pluginName}] Load failed, attempting backup:`, e);
        ...
        JsonEx.stringify = function(object) {
            try {
                ...
                console.error(`[${pluginName}] JSON stringify failed, using safe stringify`);
        ...
        console.log(`[${pluginName}] Save system fixes applied`);
    }
    ...
    if (Config.enableEventSystemFixes && typeof Game_Character !== 'undefined') {
        ...
        Game_Interpreter.prototype.update = function() {
            ...
            if (this._loopCounter > 100000) {
                console.error(`[${pluginName}] Event interpreter loop limit exceeded - terminating`);
                this.terminate();
                this._loopCounter = 0;
                return;
            }
        ...
        console.log(`[${pluginName}] Event system fixes applied`);
    }
    ...
    if (Config.enableBattleSystemFixes && typeof BattleManager !== 'undefined') {
        ...
        console.log(`[${pluginName}] Battle system fixes applied`);
    }
    ...
    */
    ...
    if (Config.enableRendererFixes) {
        console.log(`[${pluginName}] Renderer fixes SKIPPED (disabled for compatibility)`);
    }
    ...
    if (Config.enableTextureBleedingFix && typeof PIXI !== 'undefined') {
        ...
        console.log(`[${pluginName}] Texture bleeding fix applied`);
    }
    ...
    if (Config.enableLeakDetection && typeof Bitmap !== 'undefined' && typeof Sprite !== 'undefined') {
        ...
        MemoryLeakTracker.checkForLeaks = function() {
            ...
            if (warnings.length > 0) {
                if (Config.enableErrorLogging) {
                    console.warn(`[${pluginName}] === MEMORY LEAK DETECTION ===`);
                    warnings.forEach(w => console.warn(`  • ${w}`));
                    console.warn('Check for undisposed bitmaps, sprites, or event listeners');
                }
        ...
        window.checkMemoryLeaks = function() {
            MemoryLeakTracker.checkForLeaks();
            console.log('Memory Tracker Status:', MemoryLeakTracker.counts);
            return {
        ...
        console.log(`[${pluginName}] Memory leak detection enabled (dev mode)`);
    }
    ...
    if (Config.enableFontLoadingFix && typeof Bitmap !== 'undefined') {
        ...
        console.log(`[${pluginName}] Font loading fix applied`);
    }
    ...
    if (Config.enableWindowThrottling && typeof Window_Base !== 'undefined') {
        ...
        console.log(`[${pluginName}] Window refresh throttling applied`);
    }
    ...
    const TransitionManager = {
        ...
        createFadeSprite() {
            try {
                ...
                console.warn(`[${pluginName}] Failed to create fade sprite:`, e);
                this.fading = false;
            }
        },
        ...
        completeFade() {
            ...
            if (this.fadeType === 'out' && callback) {
                try {
                    ...
                    console.error(`[${pluginName}] Transition callback error:`, e);
                }
            }
    ...
    if (Config.enableSmoothTransitions && typeof Scene_Base !== 'undefined' && typeof SceneManager !== 'undefined') {
        ...
        console.log(`[${pluginName}] Smooth scene transitions applied`);
    }
    ...
    class MZPatchHub {
        ...
        initialize() {
            ...
            console.log(`[${pluginName}] Performance Hub initialized`);
        }
        ...
        processFrameBudget() {
            ...
            while (this.deferredOperations.length > 0) {
                ...
                try {
                    ...
                    console.warn(`[${pluginName}] Deferred operation failed:`, e);
                }
            }
        ...
        getReport() {
            ...
            console.log(`[${pluginName}] Performance Report:`, report);
        ...
        optimizeAll() {
            ...
            console.log(`[${pluginName}] Manual optimization: ${opts.join(', ')}`);
            return opts;
        }
        ...
        setQuality(level) {
            ...
            window.$qualityLevel = this.currentQualityLevel;
            console.log(`[${pluginName}] Quality: ${old} -> ${this.currentQualityLevel}`);
    ...
    if (typeof Scene_Base !== 'undefined') {
        ...
        Scene_Base.prototype.update = function() {
            ...
            if (now - lastGcHint > 300000) {
                ...
                if (Config.enableErrorLogging) {
                    console.log(`[${pluginName}] Periodic memory cleanup`);
                }
            }
        };
    }
        ...
        Scene_Boot.prototype.start = function() {
            ...
            setTimeout(() => {
                try {
                    ...
                    console.log(`[${pluginName}] v${VERSION} loaded successfully`);
                    console.log(`[${pluginName}] Commands: $mzPatch.getReport(), .optimizeAll(), .setQuality(1-5), .toggleHUD(), .getLeakStatus()`);
                    if (Config.enableLeakDetection) {
                        console.log(`[${pluginName}] Leak detection ON - use checkMemoryLeaks() for manual check`);
                    }
                    if (Config.enableTextureBleedingFix) {
                        console.log(`[${pluginName}] Texture bleeding fix active`);
                    }
                } catch (e) {
                    console.error(`[${pluginName}] Initialization error:`, e);
                }
            }, 100);
        };
    }
    ...
    console.log(`[UnofficialMZPatch] Plugin code executed successfully`);
    
    } catch (globalError) {
        // Catch any errors during plugin initialization
        console.error('[UnofficialMZPatch] CRITICAL ERROR during plugin load:', globalError);
        console.error('[UnofficialMZPatch] Stack trace:', globalError.stack);
        // Don't crash the game - just disable the plugin functionality
    }
