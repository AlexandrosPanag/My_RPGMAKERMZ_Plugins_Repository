...
**Memory Leak Fixes:**
- 🔧 Added `cleanup()` method to GameNwWindow class for proper resource management
- 🔧 Fixed event listener removal by storing `_clickHandler` reference
- 🔧 Added `SceneManager.terminate` hook to call cleanup on game termination
- 🔧 Added window `'close'` event handler for proper resource cleanup on window close
- 🔧 Performance monitoring interval now properly cleaned up

**API Modernization:**
- 🆕 Updated `deleteFolderRecursive()` to use modern `fs.rmSync()` API (Node.js 14.14.0+)
- 🆕 Updated `copyFolderSync()` to use modern `fs.cpSync()` API (Node.js 16.7.0+)
- 🆕 All modern APIs include fallbacks for older Node.js versions

**Performance Optimization:**
- ⚡ Optimized `executeScreenshot()` to use `canvas.toBlob()` instead of base64 conversion
- ⚡ Reduced memory pressure from screenshot operations
- ⚡ Binary blob approach eliminates large base64 string creation in memory

### v2.0.0 (2025-11-14) - Modern API Update
**Improvements:**
- 🆕 Modern NW.js API implementation
- ⚡ Performance optimization in loops and caching
- 🔧 UI improvements and better error handling

### v2.0.0 (2025-11-03) - Performance Edition
**Performance Improvements:**
- ⚡ Optimized loops and caching
